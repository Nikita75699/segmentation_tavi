...
class SegmentationModel(pl.LightningModule):
    ...
    def predict(
        ...
        images_tensor = torch.Tensor(images.transpose((0, 1, 2, 3))).to(device)
    ...
    def log_predict_model_on_epoch(
        ...
        wandb_images = []
        for idx, img_path in enumerate(glob('data/fold_3/visualization/img/*.jpg')):
            img = cv2.imread(img_path)
            img = cv2.resize(img, (self.input_size, self.input_size))
            mask = tifffile.imread(f"{img_path.replace('img', 'mask').split('.')[0]}.tif")
